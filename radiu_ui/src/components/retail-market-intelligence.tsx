  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((card, index) => {
        ...
        return (
          <Card key={index} className="border-0 shadow-xl bg-gradient-to-br from-white to-gray-50/50">
...
function MarketFactorChart({ data }: { data: MarketIntelligenceData['Market_Factor'] }) {
  const factors = [
    { name: "Rent Index", value: data.components.rent_index * 100, color: "#3B82F6" },
    { name: "Regulatory Index", value: data.components.regulatory_index * 100, color: "#10B981" },
    { name: "Seasonality Index", value: data.components.seasonality_index * 100, color: "#F59E0B" },
    { name: "Competition Density", value: data.components.competition_density * 100, color: "#EF4444" }
  ]
...
function IncomeTrendChart({ data }: { data: MarketIntelligenceData['Income_Data']['data'] }) {
  return (
    <Card className="border-0 shadow-xl bg-gradient-to-br from-white to-gray-50/50">
      <CardHeader>
        <CardTitle className="flex items-center gap-3 text-xl font-semibold">
          ...
          {data.map((item, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center gap-3">
                <span className="font-bold text-purple-600">${item.value.toLocaleString()}</span>
...
function TrafficAnalysis({ data }: { data: MarketIntelligenceData['Traffic_Score'] }) {
  return (
    <Card className="border-0 shadow-xl bg-gradient-to-br from-white to-gray-50/50">
      <CardHeader>
        <CardTitle className="flex items-center gap-3 text-xl font-semibold">
        ...
        <CardDescription>Location traffic patterns and POI categories</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <h4 className="font-semibold text-gray-800">Top POI Categories</h4>
          {data.top_poi_categories.map((poi, index) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <span className="font-medium text-gray-700">{poi.category}</span>
...
function CompetitorAnalysis({ data }: { data: MarketIntelligenceData['Existing_Competitors'] }) {
  return (
    <Card className="border-0 shadow-xl bg-gradient-to-br from-white to-gray-50/50">
      <CardHeader>
        <CardTitle className="flex items-center gap-3 text-xl font-semibold">
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
    <div className="text-center p-4 bg-orange-50 rounded-lg border border-orange-200">
      <div className="text-2xl font-bold text-orange-600">{data.data.total_competitors}</div>
  ...
  <div className="space-y-3">
    ...
    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
      <div className="font-medium text-gray-800">{data.data.statistics.closest.name}</div>
      ...
      {data.data.competitors.map((comp: any, idx: number) => (
        ...
        >
          ...
          <div className="text-xs text-blue-600">
            <a href={comp.google_maps_url} target="_blank" rel="noopener noreferrer">
              View on Map
            </a>
          </div>
        </div>
...
export function RetailMarketIntelligence({data, lat, lon, loading, error}: {data?: MarketIntelligenceData, lat?: string, lon?: string, loading?: boolean, error?: string}) {
  const currentLocation = lat && lon ? `${Number.parseFloat(lat).toFixed(4)}, ${Number.parseFloat(lon).toFixed(4)}` : "No location set"
  ...
  return (
    <div className="space-y-8">
      ...
      <CardTitle className="text-4xl font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
        ...
        <p className="text-gray-600">Comprehensive market analysis for your business location</p>
      ...
      {/* Error Display */}
      {error && (
        <Alert variant="destructive" className="animate-fade-in-up border-red-200 bg-red-50">
          ...
          <Card className="border-0 shadow-xl bg-gradient-to-br from-white to-pink-50/50">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between p-4 rounded-lg bg-gradient-to-r from-pink-50 to-pink-100 border border-pink-200">
                <span className="text-base font-semibold text-pink-800">Cultural Fit Score</span>
